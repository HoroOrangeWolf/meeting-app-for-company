...
import { Text, Button, Icon, Center, Spinner } from "native-base";
...
export default function Users({ navigation }) {
  ...
  const onLongPress = (item) => {
    Alert.alert(
      `Czy chcesz ${
        item.isBlock ? "odblokować" : "zablokować"
      } tego użytkownika?`,
      `Email: ${item.email}`,
      [{ text: "Tak", onPress: () => block(item) }, { text: "Nie" }]
    );
  };
  ...
  return isLoading ? (
    ...
    <UserList data={users} onLongPress={onLongPress} />
  );
}
