...
export default function GlobalContext({ children }) {
  ...
  const updateUser = async () => {
    ...
    await updateDoc(doc(database, "users", user.external.id), {
      ...user.external,
    });
  };
  ...
  const leaveMeeting = async (meetingId) => {
    user.external.meetings = user.external.meetings.filter(
      (f) => f !== meetingId
    );
    await updateUser();
  };
  ...
  const isMeetingOwner = (meetingId) => {
    return (
      meetingBuff.find((f) => f.id === meetingId).email === user.external.email
    );
  };
  ...
  const addMeetinng = async (meeting) => {
    ...
    const docRef = await addDoc(collection(database, "meeting"), {
      name,
      description,
      timeDate,
  ...
  const getMeetings = async () => {
    const arrayBuff = [];
    const q = query(
      collection(database, "meeting"),
      where(documentId(), "in", [...user.external.meetings])
    );
    ...
    const found = user.external.meetings.filter(
      (f) => arrayBuff.find((rec) => rec.id === f) != undefined
    );
  ...
  const updateMeeting = async (meeting) => {
    ...
    await updateDoc(doc(database, "meeting", id), {
      ...meeting,
