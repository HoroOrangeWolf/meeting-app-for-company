...
export default function HomeTab({ navigation }) {
  ...
  useEffect(() => {
    ...
    getMeetings()
      ...
      .catch((exc) => {
        console.log(exc);
        Alert.alert("Error!", "Nie mozna pobrać spotkań.", [{ text: "OK" }]);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const deleteMeeting = (itemId) => {
    ...
    removeMeeting(itemId)
      ...
      .catch(() => {
        Alert.alert("Error!", "Nie można usunąć spotkania.", [{ text: "OK" }]);
      })
      .finally(() => {
        setLoading(true);
      });
  ...
  const leaveMeetingInside = (itemId) => {
    ...
    leaveMeeting(itemId)
      ...
      .catch((exc) => {
        Alert.alert("Error!", "Opuścić spotkania.", [{ text: "OK" }]);
      })
      .finally(() => {
        setLoading(true);
      });
  ...
  const longPress = (item) => {
    if (isMeetingOwner(item.id) && isAdmin()) {
      Alert.alert("Usuwanie Spotkania", `Czy chcesz usunąć: ${item.name}?`, [
        { text: "Tak", onPress: () => deleteMeeting(item.id) },
        { text: "Nie" },
      ]);
    } else {
      Alert.alert(
        ...
        [
          { text: "Tak", onPress: () => leaveMeetingInside(item.id) },
          { text: "Nie" },
        ]
      );
    }
  ...
  const onItemModification = async (modifiedItem) => {
    ...
    updateMeeting(modifiedItem)
      ...
      .catch((exc) => {
        console.log(exc);
        Alert.alert("Błąd", `Wystąpił błąd podczas modyfikowania spotkania`, [
          { text: "OK" },
        ]);
      })
      .finally(() => {
  ...
  return isLoading ? (
    ...
    <EditMeeting
      onMeetingMody={onItemModification}
      singleMeeting={itemToModify}
      onBack={onBack}
      isEditBlocked={!isAdmin()}
    />
  ) : (
    <Box
      w={{
