...
export default function LogScreen({ navigation }) {
  const {
    globalStyles: { label, input, labelText, button, logRegScreen, buttonText },
    loginUser,
  } = useGlobalContext();
        ...
        <Stack space={2}>
          ...
          <Box style={{ marginTop: 25 }}>
            <Button
              ...
              onPress={() => {
                setLogging(true);
                handleLogin(logInData.login, logInData.password).catch(
                  (exc) => {
                    if (exc.message === "User is blocked!") {
                      Alert.alert("Error!", "Konto jest zablokowane!", [
                        { text: "OK" },
                      ]);
                    } else {
                      Alert.alert("Error!", "Email lub hasło niepoprawne!", [
                        { text: "OK" },
                      ]);
                    }
