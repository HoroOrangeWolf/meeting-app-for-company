...
export default function MeetingEditor({ navigation, route }) {
  ...
  const [meeting, setMeeting] = useState({
    ...singleMeeting,
    timeDate: new Date(singleMeeting.timeDate),
  });
        ...
        <FormControl style={{ width: "80%" }}>
          <Stack space={2}>
            ...
            <Button
              style={button}
              onPress={() =>
                navigation.navigate("QRCodeGenerator", { value: meeting.id })
              }
            >
              <Text style={buttonText}>Wygeneruj kod QR</Text>
            </Button>
            {isModified && !isEditBlocked && (
              <Button style={button} onPress={() => onMeetingMody(meeting)}>
